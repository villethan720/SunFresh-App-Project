document.addEventListener("DOMContentLoaded", function() {
    ...
    //const apiUrl = "http://localhost:3009/api/v1/price/";

    //gets the api endpoint to access the information in database
    fetch(apiUrl)
        .then((response) => {
            //when there is data being accessed, the response is then parsed as json
            if (response.ok) {
                return response.json();
            //when no data is accessed then throws an error message
            } else {
  ...
  dropdown.addEventListener("change", function(event) {
      ...
      //API
      //fetch("http://localhost:3009/api/v1/inventory/" + selectedValue)
      //gets in data through api where the selected value is stored at (in inventory)
    ...
    //function to show the data about the cocktails.
    function displayCocktail(data) {
      ...
      txtprice.value = cocktail.price;
      document.getElementById("price").value = cocktail.price;
      ...
      */
        ...
        if(item_code.value.substring(0,1) == "z"){
            txttaxpercent.value = 7;
            document.getElementById("txttaxpercent").value = 7;
         ...
         */
         ...
         //output on screen
         document.getElementById("txttotalamt").value = itemtotalprice;
   ...
   // document.getElementById('txtqty').addEventListener('input', function(event) {
    ...
    function yourFunction(value) {
        ...
        console.log('The function was triggered with value:', value);
    }
